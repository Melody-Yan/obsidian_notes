...
var SlurpPlugin = class extends import_obsidian.Plugin {
  constructor() {
    super(...arguments);
    this.errorNotice = (err) => new import_obsidian.Notice(`Slurp Error! ${err.message}`);
  }
  async onload() {
  ...
  fixRelativeLinks(html, articleUrl) {
  ...
  async fetchHtml(url) {
    const html = await (0, import_obsidian.requestUrl)(url).text;
    if (!html) {
  ...
  parseMarkdown(content) {
    const md = (0, import_obsidian.htmlToMarkdown)((0, import_obsidian.sanitizeHTMLToDom)(content));
    if (!md) {
  ...
  async createFilePath(title) {
    const fpLoop = (p, fn, retries) => {
      if (retries == 100)
        throw "Cowardly refusing to increment past 100.";
      const suffix = retries > 0 ? `-${retries}.md` : ".md";
      const fp = (0, import_obsidian.normalizePath)(`${p}/${fn}${suffix}`);
      return this.app.vault.getFileByPath(fp) ? fpLoop(p, fn, retries + 1) : fp;
    };
    const folder = this.app.vault.getFolderByPath("Slurped Pages") || await this.app.vault.createFolder("Slurped Pages");
    const fileName = title.replace(/[\\\/:]/g, "-");
    return fpLoop(folder.path, fileName, 0);
  }
  createContent(url, article) {
    const fmtDt = (dt, time) => {
      return (0, import_obsidian.moment)(dt || new Date()).format(time ? "YYYY-MM-DDTHH:mm" : "YYYY-MM-DD");
    };
    const notIfEmpty = (k, v) => v ? `${k}: ${v}
` : "";
    const publishedTime = article.publishedTime ? fmtDt(article.publishedTime) : void 0;
    let noteProps = "---\n";
    noteProps += `link: ${url}
`;
...
`;
  ...
  async slurpNewNoteCallback(args) {
    var _a;
    if (args.err)
      throw args.err;
    if (!args.article)
      throw "Article is empty.";
    ...
    (_a = this.app.workspace.getActiveViewOfType(import_obsidian.MarkdownView)) == null ? void 0 : _a.leaf.openFile(newFile);
  }
};
var SlurpNewNoteModal = class extends import_obsidian.Modal {
  ...
  onOpen() {
    ...
    const urlField = new import_obsidian.TextComponent(contentEl).setPlaceholder("URL").onChange((val) => this.url = val);
    urlField.inputEl.setCssProps({ "width": "100%" });
    const progressBar = new import_obsidian.ProgressBarComponent(contentEl);
    ...
    const doSlurp = async () => {
      ...
      try {
        ...
        this.plugin.errorNotice(err);
      }
      clearInterval(t);
      this.close();
    };
    new import_obsidian.Setting(contentEl).addButton(
      (btn) => btn.setButtonText("Slurp").setCta().onClick(doSlurp)
    );
